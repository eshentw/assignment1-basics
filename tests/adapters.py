...
from cs336_basics.optimizer import AdamW, cosine_lr_scheduling, gradient_clipping
from cs336_basics.dataset import data_loading
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    self = SelfAttention(d_model=d_model, num_heads=num_heads, theta=theta, max_seq_len=max_seq_len)
    self.q_proj.load_state_dict({"weight": q_proj_weight})
    self.k_proj.load_state_dict({"weight": k_proj_weight})
    self.v_proj.load_state_dict({"weight": v_proj_weight})
    self.o_proj.load_state_dict({"weight": o_proj_weight})
...
def run_get_batch(
    ...
    """
    return data_loading(dataset, batch_size, context_length, torch.device(device)).__next__()
